...
| Base zoning | `Zone_Update_2025___Related_Master/2` | Whole-parcel coverage (from 10 September 2026): boundary intersection for candidates, a multipoint of interior probe points for which of them cover area inside the parcel, and `esriSpatialRelWithin` for one district containing the whole parcel. Features are grouped by `ZONE_`, so adjacent same-code polygons are one designation. Every candidate is reported; an unconfirmed one is labelled as a touch. No proportion is measured or published. Fields shown (from 10 September 2026, PR A4): `ZONE_` (Code), `Category`, `Zone_Desc1` (What this district is for — a purpose sentence), `CodeRef` (Code section) and `CodebookURL` (Read the code section, a link to municipalcodeonline.com). `ZONE_DESC`, the all-caps duplicate of `Category`, and `Res_Max_De` are not shown. `ZoneName` is also not shown: verified 10 September 2026, it says `C` where `ZONE_` says `C-2` — that inconsistency is in the published data, not this page, and is worth raising with Planning before `ZoneName` is ever surfaced | Millcreek Planning and GIS; source used by public Planning web map | Verify after every zoning-map amendment and against web-map parity check; the `zoning-coverage-known-parcel`, `zoning-boundary-overlap`, `zoning-gap-parcel` and `spatial-relation-direction` live contracts must hold |
| Future land use | `FutureLandUse_2024_Millcreek/0` | Whole-parcel coverage, identical method to base zoning; features grouped by `LandUse`. Expected to cover every parcel, so no polygon is reported as a gap in the map rather than as a property without a designation | Millcreek Planning and GIS; source used by public Planning web map | Verify against the adopted General Plan and web-map parity check |
...
| City Center Overlay | `Zone_TCOZ/0` | Whole-parcel coverage, reported as Yes / No / Unknown. Optional overlay, so a complete query that finds nothing is a real No. The layer has no designation field — membership is the whole answer — so every returned feature maps to one constant local key. A boundary touch no probe confirms reads Unknown, never Yes | Millcreek Planning and GIS | Verify after overlay amendments |
...
**Post-deploy check.** `npm run check:deployment` was red from 13 to 26 August 2026 — Netlify's
...
including the known hazard parcel, the fault special-study area, three informational hazard
screenings, the two historic designation types, the STR parcel and 400-foot buffer, and parity with
the public Planning web map across 21 adopted local layers. No source URL, field or domain changed;
this substantiates `CFG.release.dataReviewedOn = 2026-08-13` rather than recording a migration.
